...
function bloggerFormatla(ham) {
  if (!ham) return ""; // içerik yoksa (örn. bazı video feed'leri) sessizce boş bırak, "hata" gibi görünen metin gösterme
...
async function ogeyiGonder(feed, item, guid, kaynakTuru) {
  const hamIcerik = item.contentEncoded || item.content || item.mediaDescription || item.contentSnippet || "";
